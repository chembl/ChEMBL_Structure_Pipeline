...
vanadium_to_keep = ['exclude_mols/v1.mol', 'exclude_mols/v2.mol', 'exclude_mols/v3.mol', 'exclude_mols/v4.mol']
...
def exclude_flag(molfile):
    ...
    matches = [mol.HasSubstructMatch(Chem.MolFromMolFile(x, sanitize=False)) for x in mols_to_keep]
    if True in matches:
        return exclude

    # structures with vanadium to keep
    matches = [mol.HasSubstructMatch(Chem.MolFromMolFile(x, sanitize=False)) for x in vanadium_to_keep]
